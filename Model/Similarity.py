...
class Similarity(nn.Module):
    def __init__(self, drug_nodes, cell_nodes, drug_edges, cell_edges, args):
        ...
        self.drug_nodes_feature = drug_nodes.to(args.device) # torch.Size([170, 384])
        self.drug_edges = drug_edges.to(args.device)

        self.cell_nodes_feature = cell_nodes.to(args.device) # torch.Size([580, 2944])
        self.cell_edges = cell_edges.to(args.device)

        self.dropout = nn.Dropout(args.dropout_ratio)
        self.dropout_ratio = args.dropout_ratio

        self.dim_cell = cell_nodes.size(1) # 2944
        self.dim_drug = drug_nodes.size(1) # 384
    ...
    def forward(self, drug, cell):
        '''
        drug: tensor([mixed drug indexes]) 
        cell: tensor([mixed cell line indexes]) 
        * tensor sizes are same with batch size
        '''
        drug_id = drug.long() # Convert index saving tensor 'drug' into long type
        cell_id = cell.long()
